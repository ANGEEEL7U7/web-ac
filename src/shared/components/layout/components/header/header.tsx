import { ActionMenu, ScrollBehavior } from "./header.hook";
import { Braces, GraduationCap, Moon, Sun, UserStar } from 'lucide-react';
import './style-header.css'
import { UseTheme } from "@shared/providers";

export default function Header() {
    const [scrollActivo] = ScrollBehavior();
    const [menuStatus, toggleMenu] = ActionMenu();
        ...
        <nav>
            ...
            <div className="header-navigation">
                <div className="header-buttons">
                    <button className="button-header-nav">Estadistica</button>
            ...
            <div className="header-navigation-small">
                <div className="header-buttons">
                    <button className="group button-header-nav focus icon-header ">
                        <span className="label-icon-header ">Estadistica</span>
                        <UserStar size={18} className="icon-button-header" />
                    </button>
                    <button className="group button-header-nav focus icon-header ">
                        <span className="label-icon-header ">Competencias</span>
                        <Braces size={18} className="icon-button-header" />
                    </button>
                    <button className="group button-header-nav focus icon-header ">
                        <span className="label-icon-header ">Ediciones</span>
                        <GraduationCap size={18} className="icon-button-header" />
                    </button>
                </div>
                <button onClick={changeTheme} className="button icon transition-all">
                    {/*theme === 'dark' */}
...
/*
 <div className="flex flex-col pr-3 pl-4 w-full whitespace-nowrap visible lg:hidden">
                <div className="flex  h-full w-full">
                    <button className='button icon transition-all' onClick={toggleMenu}>
                        <span
                    ...
                    <div className={`${menuStatus ? 'h-100' : 'h-0'} overflow-auto transition-[height] duration-400 break-all whitespace-normal`}>
